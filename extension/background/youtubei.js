...
export async function fetchComments(videoId) {
  ...
  while (
    ...
    const items =
      pageCount === 0
        ? commentsResponse.onResponseReceivedEndpoints[1]
            .reloadContinuationItemsCommand.continuationItems
        : commentsResponse.onResponseReceivedEndpoints[0]
    ...
    for (const item of items) {
      if (item.commentThreadRenderer) {
        ...
        if (commentThreadRenderer.comment) {
          ...
          const text = cr.contentText.runs.map((run) => run.text).join("");
          comments.push({
            commentId,
            authorName,
            authorAvatar,
          ...
          const mutation =
            commentsResponse.frameworkUpdates.entityBatchUpdate.mutations.find(
              (e) => e.entityKey === commentKey,
            );
...
function commentsContinuationToken(videoResponse) {
  ...
  return response.contents.twoColumnWatchNextResults.results.results.contents.find(
    (e) =>
      ...
      e.itemSectionRenderer.sectionIdentifier === "comment-item-section",
  ).itemSectionRenderer.contents[0].continuationItemRenderer // When comments are disabled there is messageRenderer instead.
